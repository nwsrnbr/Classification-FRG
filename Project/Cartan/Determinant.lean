...
This file computes determinants of Cartan matrices for simple Lie algebras, both the exceptional types
...
lemma det_E₅ : E₅.det = 4 := by
  ...
  · simp [det_E₄, det_E₃]
  · ext i j
    simp [ind_matrix, E₅, E₄, Fin.castLT]
    fin_cases i
    <;> fin_cases j
    <;> simp
  · ext i j
    simp [isTopLeftBlock, E₄, E₃]
    fin_cases i
    <;> fin_cases j
    <;> simp
...
theorem det_A (n : ℕ) : det (A n) = n + 1 := by
...
theorem det_B (n : ℕ) : det (B n) = if n = 0 then 1 else 2 := by
  induction' n using Nat.strongRec with n ih
  cases n with
  | zero => simp
  | succ n =>
