...
import Project.Cartan.ExtendedCartan
import Project.RootSystem.Determinant
...
theorem det_SymmMatrix_B' : (SymmMatrix (B' n)).det = if n = 0 ∨ n = 1 then 2 else if n = 2 then 4 else 0 := by
  ...
  | succ n =>
    cases n with
    | zero =>
      have : SymmMatrix (B' 1) = !![2, -√2; -√2, 2] := by
        simp [SymmMatrix, B']
        ext i j
        fin_cases i
        <;> fin_cases j
    ...
    | succ n =>
      ...
      by_cases hn : n = 0
      · rw [hn]
        have : SymmMatrix (B' 2) = !![2, 0, 0; 0, 2, -√2; 0, -√2, 2] := by
          simp [SymmMatrix, B', D_rev]
          ext i j
          fin_cases i
          <;> fin_cases j
          <;> simp
        ...
        grind
      · rw [ind_det (SymmMatrix (B' (n + 1 + 1))) (SymmMatrix (D_rev (n  + 1 + 1))) (SymmMatrix (D_rev (n + 1))) (-√2 : ℝ) (-√2 : ℝ)]
        · simp [det_SymmMatrix_D_rev, det_SymmMatrix_D]
          aesop
        · ext i j
          simp [SymmMatrix, ind_matrix, B', Fin.castLT]
          by_cases hi : i ≤ n + 1
          by_cases hj : j ≤ n + 1
          · simp [hi, hj]
            grind
...
theorem det_SymmMatrix_C' : (SymmMatrix (C' n)).det = if n = 0 ∨ n = 1 then 2 else 0 := by
  sorry
  /-
  induction' n using Nat.strongRec with n ih
  cases n with
  ...
  | succ n =>
    cases n with
    | zero =>
      have : SymmMatrix (C' 1) = !![2, -√2; -√2, 2] := by
        simp [SymmMatrix, C']
        ext i j
        fin_cases i
        <;> fin_cases j
    ...
    | succ n =>
      ...
      by_cases hn : n = 0
      · rw [hn]
        have : SymmMatrix (C' 2) = !![2, -√2, 0; -√2, 2, -√2; 0, -√2, 2] := by
          simp [SymmMatrix, C', C]
          ext i j
          fin_cases i
          <;> fin_cases j
        ...
        simp [this, Matrix.det_fin_three]
        grind
      · rw [ind_det (SymmMatrix (C' (n + 1 + 1))) (SymmMatrix (C_rev (n  + 1 + 1))) (SymmMatrix (C_rev (n + 1))) (-√2 : ℝ) (-√2 : ℝ)]
        · simp [det_SymmMatrix_C_rev]
        · ext i j
          simp [SymmMatrix, ind_matrix, C', Fin.castLT]
          by_cases hi : i ≤ n + 1
          by_cases hj : j ≤ n + 1
          · simp [hi, hj]
            grind
